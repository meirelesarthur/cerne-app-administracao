...
class OrdemServicoStoreNotifier extends Notifier<OrdemServicoState> {
  ...
  String criar({
    ...
    required String responsavelExecucao,
    ...
    String? culturaVariedade,
    String? lote,
    String? categoria,
    required CondicoesOs condicoes,
    required String descricao,
    required InstrucoesOs instrucoes,
    required SegurancaOs seguranca,
    required List<MaoDeObraOs> maoDeObra,
    required List<MaquinaOs> maquinas,
    required String armazemInsumos,
    required List<InsumoOs> insumos,
    String? armazemProducao,
    required List<ProducaoOs> producao,
    required List<EpiOs> epis,
    required String autor,
  }) {
    final agora = DateTime.now();
    final codigo = 'OS #${_proximoNumero()}';
    final nova = OrdemServico(
      id: 'os-${agora.microsecondsSinceEpoch}',
      codigo: codigo,
      fazenda: fazenda,
      solicitante: autor,
      ...
      seguranca: seguranca,
      maoDeObra: List.unmodifiable(maoDeObra),
      maquinas: List.unmodifiable(maquinas),
      armazemInsumos: armazemInsumos,
      insumos: List.unmodifiable(insumos),
      armazemProducao: armazemProducao,
      producao: List.unmodifiable(producao),
      epis: List.unmodifiable(epis),
      autorizador: autor,
      dataAutorizacao: agora,
      // Prioridade faz parte do ciclo administrativo, não dos 34 campos do
      // formulário WEB. Mantém a opção padrão histórica da fila do ADM.
