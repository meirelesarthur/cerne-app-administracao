...
void main() {
  ...
  for (final MapEntry(key: nome, value: s) in themes.entries) {
    group('Contraste — tema $nome', () {
      ...
      test('placeholder sobre os preenchimentos de campo ≥ 4,5:1', () {
        expect(
      ...
      final status = {
        'success': (s.statusSuccessFg, s.statusSuccessBg),
        'warning': (s.statusWarningFg, s.statusWarningBg),
        'danger': (s.statusDangerFg, s.statusDangerBg),
        'info': (s.statusInfoFg, s.statusInfoBg),
        'neutral': (s.statusNeutralFg, s.statusNeutralBg),
      };
      for (final st in status.entries) {
        test('status ${st.key}: texto sobre o fundo suave e sobre surface', () {
